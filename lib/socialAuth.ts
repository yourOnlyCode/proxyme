...
export async function linkWithProvider(provider: OAuthProvider) {
  ...
  console.log('[oauth] link start', { provider, returnUrl, redirectTo });
  ...
  const result = await WebBrowser.openAuthSessionAsync(data.url, returnUrl);
  console.log('[oauth] link browser result', result);
  if (result.type !== 'success' || !result.url) {
    throw new Error(`OAuth cancelled (${result.type}).`);
  }
  ...
  // Some providers may return tokens directly (hash fragment). Support that as a fallback.
  const accessToken = getParam(result.url, 'access_token');
  const refreshToken = getParam(result.url, 'refresh_token');
  if (accessToken && refreshToken) {
    const { error: setSessionError } = await supabase.auth.setSession({
